...
!        end if
        wmin = 1
        wmax = nrig       ! suppose nrig == n water molecules
        nfree = nliqa - nriga
...
!        end if
        ...
        qmass = real(3*(nfree+nrig))*tstar*2e4
        rmass = real(3*nrig)*tstar*1e4
        qmass = qmass/10.0
        rmass = rmass/10.0
...
!        print *, 'srs', s0, rs0
    ...
    subroutine move_shake_init
        ...
        x1(nmove+1:ntota) = x0(nmove+1:ntota)
        y1(nmove+1:ntota) = y0(nmove+1:ntota)
        z1(nmove+1:ntota) = z0(nmove+1:ntota)
                    ...
                    if (moved(ia)*moved(ib) == 0) then
                        ...
                        if (abs(diffsq) > rabsq*tol2) then
                            ...
                            if (rpab < rabsq*rptol) then
                                ...
                                write(*, *) y0(ia), y0(ib)
                                write(*, *) z0(ia), z0(ib)
                                write(*, *) sqrt(pxab*pxab + pyab*pyab + pzab*pzab), sqrt(rabsq)
