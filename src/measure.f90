module measure
    ...
    real*8, dimension(:, :), allocatable :: efipos, tefipos
    integer*8, dimension(:, :), allocatable :: nefipos
    ...
    subroutine measure_init
        ...
        allocate(efipos(1:2, 0:lmax), tefipos(1:2, 0:lmax), nefipos(1:2, 0:lmax))
...
!        print *, 'a'
        ...
        if (.true.) then
            ...
            call measure_gather_dble2d(efipos)
        end if
...
!            end do
            ...
            write(4, *) '# efield'
            where (nefipos(1,:) > 0) efipos(1,:) = efipos(1,:)/real(nefipos(1,:))
            where (nefipos(2,:) > 0) efipos(2,:) = efipos(2,:)/real(nefipos(2,:))
            where (nefipos(1,:) > 0) efipos(1,:) = tefipos(1,:)/real(nefipos(1,:))
            where (nefipos(2,:) > 0) efipos(2,:) = tefipos(2,:)/real(nefipos(2,:))
            do l = 1, lmax
                r = (real(l)+0.5)*rdel
                write(4, 40) r - lwh, efipos(1,l), efipos(2,l), tefipos(1,l), tefipos(2,l)
            end do
            end if
