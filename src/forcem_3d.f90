...
module force  ! state of the system : configuration, energy, forces, ...
    ...
    ! fouriere part
    integer*4 :: kmax, klmax, kmmax, knmax, iqmax ! number of k-vectors and charges
...
!        print *, 'f'
!        print *, 'sum1', sum(abs(fx(1:nriga))), sum(abs(fy(1:nriga))), sum(abs(fz(1:nriga)))
        call force_real
        !write(*,'(i1,3(1x,f10.3))') 1,sum(abs(fx)), sum(abs(fy)), sum(abs(fz)) 
!        print *, 'g'
!        print *, 'sum2', sum(abs(fx(1:nriga))), sum(abs(fy(1:nriga))), sum(abs(fz(1:nriga)))
        call force_fouri
        !write(*,'(i1,3(1x,f10.3))') 3,sum(abs(fx)), sum(abs(fy)), sum(abs(fz)) 
!        print *, 'i'
!        print *, 'sum3', sum(abs(fx(1:nriga))), sum(abs(fy(1:nriga))), sum(abs(fz(1:nriga)))
        call force_intra
!        print *, 'sum4', sum(abs(fx(1:nriga))), sum(abs(fy(1:nriga))), sum(abs(fz(1:nriga)))
...
!        print *, 'sumy', sum(abs(fx(1:nriga))), sum(abs(fy(1:nriga))), sum(abs(fz(1:nriga)))
...
!        print *, 'sumz', sum(abs(fx(1:nriga))), sum(abs(fy(1:nriga))), sum(abs(fz(1:nriga)))
...
!        print *,'Q', sum(Qsumw1 + Qsumw2)
        ...
        do it = 1, ntype        ! moltypes
            select case (moltype(it))
                case ('rigmol')             ! rigmol selfinteraction
                ...
                case ('surfmol')             ! constrained atoms selfinteraction
                    do iq = 1, nt(it)
                        ix = ix + 1
                        do is = 1, nconst(it)
                            i = constra(ix, is)
...
!        print *, 'sumc', sum(abs(fcx(1:nrig))), sum(abs(fcy(1:nrig))), sum(abs(fcz(1:nrig)))
        end if
!        print *, 'sumd', sum(abs(fcx(1:nrig))), sum(abs(fcy(1:nrig))), sum(abs(fcz(1:nrig)))
